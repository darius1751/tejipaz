// 'use client'
import { useTranslations } from "next-intl";
import { NavbarLink } from "../Navbar/NavbarLink";
type Props = {
    open: boolean;
}
export const SidebarLinks = ({ open }: Props) => {
    const t = useTranslations(`RootLayout`);
    return (
        <aside className={`fixed right-0 h-[calc(100vh-5rem)] z-30 bg-black top-20 transition-all text-white duration-200 overflow-hidden ${open ? 'w-full md:w-96' : 'w-0'}`}>
            <div className="flex flex-col gap-6 pt-20 px-4 ">
                <ul className="flex flex-col gap-4 text-nowrap items-end">
                    {
                        t?.raw?.('Navbar')?.map(({ text, to }: { text: string, to: string }, i: number) => (
                            <li key={i}>
                                <NavbarLink text={text} to={to} className="text-nowrap" />
                            </li>
                        ))
                    }

                </ul>
            </div>
        </aside>
    )
}